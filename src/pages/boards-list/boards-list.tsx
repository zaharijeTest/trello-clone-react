...
import { getRecentBoards, pushRecentBoard } from "../../core/services/cache.service";
...
const BoardsListPage = ({ getBoardsAction, boards, user, selectBoardAction, createBoardAction }: IBoardListProps) => {
    ...
    return (
        <div className="boards-list-wrapper">
            ...
            <div>
                <Title><span>Recently Viewed</span></Title>
                <div className="boards-list">
                    {boards.filter(board => recentBoards.includes(board.id)).map(board => createBoardTile(board, history))}
                </div>
                <Title><span>{user?.fullName}'s Workspace</span></Title>
                <div className="boards-list">
                    {boards.map(board => createBoardTile(board, history))}
                    <div className="black-text board-tile align-center " onClick={() => {
                        setModal(true)
