...
const NewBoard: FunctionComponent<INewBoardProps> = ({
  user,
  boards,
  onSubmit,
}) => {
  ...
  return (
    <div className="new-board-wrapper">
      <div className="flex">
        <div
          className="new-board-title-wrapper small-padding small-font"
          style={{ backgroundColor: board.backgroundColor, width: "300px" }}
        >
          <Input
            style={{ color: "white" }}
        ...
        <div className="color-chooser-wrapper">
          <ColorChooser
            ...
            onColorChanged={(backgroundColor) =>
              setBoard({ ...board, backgroundColor })
            }
          />
        </div>
      </div>
      <div className="new-board-info-wrapper small-font">
        <p className="bold white-text">
          You have used {boards.length} of 10 boards
        </p>
        <p className="new-board-info-additional">
          For unlimited Boards and Power-Ups, priority support, automation and
          more, upgrade to Business Class.
        </p>
      </div>
      <div>
        <Button
          disabled={!board.name || board.name.length === 0}
          onClicked={() => onSubmit(board)}
        >
...
const mapProps: IMapToProps<any, any> = [(store) => ({}), {}];
